...
class DataWriter():
	...
	def write_image(self, img,output_prediction, im_name, stream=None):
		...
		if self.opt.save_img:
			...
			text_x = (int(width/2))-170
		

			# check the output & based on that put rect box color

			if output_prediction == "Alert":
				img1=cv2.putText(img,output_prediction,(text_x,250),cv2.FONT_HERSHEY_TRIPLEX,4,(0, 0, 255),5,cv2.LINE_AA)	
				img1=cv2.rectangle(img,(0,0),(width,height),(0,0,255),20)            # Red color rectbox
				print("putting text sucess")
				cv2.imwrite(os.path.join(self.opt.outputpath, 'vis', im_name), img1)

			else:
				img1=cv2.putText(img,output_prediction,(text_x,250),cv2.FONT_HERSHEY_TRIPLEX,4,(0, 0, 255),5,cv2.LINE_AA)
				img1=cv2.rectangle(img,(0,0),(width,height),(0,255,0),20)            # Green color rectbox
				print("putting text sucess")
				cv2.imwrite(os.path.join(self.opt.outputpath, 'vis', im_name), img1)
