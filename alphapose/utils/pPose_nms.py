...
def write_json(all_results,number, outputpath, form=None, for_eval=False):
	...
	if form == 'cmu': # the form of CMU-Pose
		...
		# Saving each iteration datalength in global variable
		global data_len_each_time
		data_len_each_time.append(raw_data_length)
		print(data_len_each_time)
		...
		if n == 0:
			for i in range(0,raw_data_length):
				demo = x.iloc[i]
				demo = list(demo)
				data_for_prediction.append(demo)
			
		else:
			old_datalength = data_len_each_time[n-1]
			current_datalength = data_len_each_time[n]
			
			for i in range(old_datalength,current_datalength):
				demo = x.iloc[i]
				demo = list(demo)
				data_for_prediction.append(demo)
			
		
		with open(os.path.join(outputpath,'final_xgboost_home_securuty_prediction_model_21jan_new_89_87.pickle'), 'rb') as xgmodel_file:
			loaded_model = pickle.load(xgmodel_file) 
		
		with open(os.path.join(outputpath,'final_xgboost_home_security_scaler_model.pickle'), 'rb') as scaler_file:
			scaler = pickle.load(scaler_file)
		
		for i in range(len(data_for_prediction)):
			test_data = data_for_prediction[i]
			prediction=loaded_model.predict(scaler.transform([test_data]))
			print("model predictio is :- ",prediction)
			results.append(prediction[0])              # saving all results in list
